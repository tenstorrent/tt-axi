...
async def test_sel_aw_frozen_while_unaccepted(dut):
    ...
    await wait_until(dut, lambda: len(mon.r_events) == 1, 20, "DECERR R during AW freeze")
    r0 = mon.r_events[0]
    assert r0["resp"] == AXI_RESP_DECERR and r0["data"] == DECERR_DATA
...
async def test_sel_ar_frozen_while_unaccepted(dut):
    ...
    await wait_until(dut, lambda: len(mon.b_events) == 1, 20, "DECERR B during AR freeze")
    b0 = mon.b_events[0]
    assert b0["resp"] == AXI_RESP_DECERR
    ...
    assert mon.r_events[1]["resp"] == AXI_RESP_DECERR
