...
/**
 * @file tb_axi_lite_isolate.sv
 * @brief Block-level harness that flattens the AXI4-Lite isolate's ports for
...
/** @brief Exposes a simulator-friendly lite-isolate harness for Cocotb tests. */
module tb_axi_lite_isolate #(
    ...
    // demux stalls a host request at its select-FIFO gates without presenting
    // it, and in that window the select is free to move (test_saturation
    // provokes exactly that, so a host-level antecedent here would be a
    // false failure).
...
`endif
    ...
    // at most 2*4 = 8 outstanding writes (W-select FIFO + B-select FIFO) and
    // 4 reads, so the inner's threshold must sit above the write bound: 9.
    ...
    initial begin
        ...
        if (u_dut.InnerPending != 32'd9) begin
            $fatal(1, "InnerPending derivation changed: expected 9 for NumPending = 4, got %0d",
                   u_dut.InnerPending);
        end
    end
...
`ifndef VERILATOR
    initial begin
        if ($test$plusargs("waves")) begin
            $dumpfile("tb_axi_lite_isolate.vcd");
            $dumpvars(0, tb_axi_lite_isolate);
        end
    end
`endif

endmodule : tb_axi_lite_isolate
