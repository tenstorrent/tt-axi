...
"""Shared utilities for the axi_lite_isolate Cocotb tests.

Provides constants, DUT state probes, AXI4-Lite channel drivers, the reactive
downstream slave model, the response monitor, and common setup. Every
test_*.py module in this directory imports from here.

AXI4-Lite has no transaction IDs and no bursts: every transaction is a single
beat and the DUT returns responses on ONE strictly ordered stream per
direction (the demux's B/R select FIFOs enforce host-side ordering). Tests
therefore account for responses by ORDER, not by ID - the monitor's b_events
and r_events line up index-for-index with the issue order.

Timing convention (matches the axi_isolate bench): everything the test
...
# For NUM_PENDING = 4: a write occupies a W-select FIFO slot from AW
# acceptance until its W beat and a B-select FIFO slot until its B, so the
# demux admits at most 8 outstanding writes; reads are bounded by the
# R-select FIFO at 4. The inner's derived threshold is 9 (checked at
# elaboration in tb_axi_lite_isolate.sv). test_saturation leans on these.
DEMUX_MAX_W = 8
DEMUX_MAX_R = 4
INNER_PENDING = 9


def rdata_for(addr: int) -> int:
    """Deterministic read payload the downstream slave returns."""
    return (addr ^ 0xA5A5_A5A5) & 0xFFFF_FFFF


# ----------------------------------------------------------------------------
# DUT state probes (observation mirrors in tb_axi_lite_isolate.sv)
# ----------------------------------------------------------------------------
...
def aw_unaccepted(dut) -> int:
    """Demux-internal AW presented-and-unaccepted (select-hold antecedent)."""
    return int(dut.obs_demux_aw_unaccepted.value)


def ar_unaccepted(dut) -> int:
    """Demux-internal AR presented-and-unaccepted (select-hold antecedent)."""
...
class DownstreamSlave:
    """Reactive AXI4-Lite slave on the DUT master port with stall controls.
    ...
    A B is owed once an AW and its W beat have both arrived; an R is owed
    per accepted AR, carrying rdata_for(addr).
    """
    ...
    @property
    def aw_count(self) -> int:
        return len(self.aw_addrs)

    @property
    def w_count(self) -> int:
        return len(self.w_beats)

    @property
    def ar_count(self) -> int:
        return len(self.ar_addrs)
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            b_show = self.release_b and (
                min(len(self.aw_addrs), len(self.w_beats)) > self.b_sent
            )
...
class RespMonitor:
    ...
    records the isolate state it completed in. With no IDs, responses
    arrive strictly in issue order: b_events[i] / r_events[i] belong to
    the i-th write / read issued.
    ...
    race the very deposit that enables a handshake on the next edge.
    """
...
async def issue_aw(dut, addr: int, cycles: int = 300) -> None:
    ...
    for _ in range(cycles):
        ...
        raise AssertionError(f"Timed out on AW handshake @0x{addr:08x}; {dbg(dut)}")
    await RisingEdge(dut.clk_i)
    dut.slv_aw_valid_i.value = 0


async def issue_w(dut, data: int, w_delay: int = 0, cycles: int = 300) -> None:
    """Drive one W beat after w_delay idle cycles."""
    for _ in range(w_delay):
        await RisingEdge(dut.clk_i)
    for _ in range(cycles):
        await RisingEdge(dut.clk_i)
        dut.slv_w_valid_i.value = 1
        dut.slv_w_data_i.value = data
        dut.slv_w_strb_i.value = 0xF
        await ReadOnly()
        if dut.slv_w_ready_o.value:
            break
    else:
        raise AssertionError(f"Timed out on W beat 0x{data:08x}; {dbg(dut)}")
    await RisingEdge(dut.clk_i)
    dut.slv_w_valid_i.value = 0


async def issue_write(dut, addr: int, data: int, w_delay: int = 0,
                      cycles: int = 300) -> None:
    """Issue AW and its W beat concurrently; B is collected by RespMonitor."""
    aw_task = cocotb.start_soon(issue_aw(dut, addr, cycles))
    w_task = cocotb.start_soon(issue_w(dut, data, w_delay, cycles))
    await aw_task
    await w_task


async def issue_read(dut, addr: int, cycles: int = 300) -> None:
    """Handshake one AR; the R beat is collected by RespMonitor."""
    for _ in range(cycles):
        await RisingEdge(dut.clk_i)
        ...
        raise AssertionError(f"Timed out on AR handshake @0x{addr:08x}; {dbg(dut)}")
    await RisingEdge(dut.clk_i)
